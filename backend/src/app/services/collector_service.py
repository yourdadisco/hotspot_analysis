        ...
        for attempt in range(max_retries):
            try:
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.get(self.feed_url, headers=headers, timeout=30, ssl=False) as response:
...
class CollectorService:
    ...
    EN_SOURCES = [
        ...
        ("VentureBeat AI", SourceType.NEWS, "https://venturebeat.com/feed/"),
    ...
    def set_language(self, lang: str):
        ...
        if lang != self._lang:
            ...
            self._init_collectors(lang)
            logger.info(f"信息源已切换至: {'中文' if lang == 'cn' else '海外'}")

    def _init_collectors(self, lang: str = "cn"):
        """初始化收集器"""
        sources = self.CN_SOURCES if lang == "cn" else self.EN_SOURCES

        for name, source_type, url in sources:
            self.collectors.append(RSSCollector(name, source_type, url))

        logger.info(f"启用 {len(sources)} 个{ '中文' if lang == 'cn' else '海外'}RSS订阅源")
